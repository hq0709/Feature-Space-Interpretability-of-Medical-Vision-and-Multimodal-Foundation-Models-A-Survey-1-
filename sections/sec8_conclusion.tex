...
\emph{Breadth is not control.} The survey therefore reports breadth separately from \Vnclasses{}
classes of safeguards, each stated in the supplement with what it licenses and what it does not
(Fig.~\ref{fig:findings} groups them five ways). No aggregate control score is formed, and a statement that lacks a class is
narrower, not refuted. The clearest bottleneck is interventional: a matched control is reported in
\Vspc{} of \Vspcn{} targeted interventions, and no medical study in this corpus meets the criterion of
\S\ref{sec:causal-claims} for a concept-specific causal contribution.

\emph{What to measure.} The next informative audit should pair a calibrated probe with a matched
intervention or an erasure at the same locus and report both. The priority is sharpened by the present
evidence base: \Fsensrefemptyn{} statements lose every record when the analysis is restricted to
peer-reviewed studies, only \Nminvalid{} of the \Ncore{} studies state an independent split, evaluate on
unused data and report uncertainty, and controls on probes are rare.
