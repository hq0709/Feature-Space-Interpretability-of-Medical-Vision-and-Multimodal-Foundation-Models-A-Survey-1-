...
reading (none a prerequisite), and where this corpus stops, with $k/n$ over the family's own records (the cross-family
counts are in the supplement). The sub-types of decoding and intervention are discussed in \S\ref{sec:decoding} and \S\ref{sec:intervention}.}
\label{tab:families}
...
The structural row ($\dagger$) covers task-linked geometry only (\S\ref{sec:method}) and is not comparable
...
Table~\ref{tab:famreq} gives what each costs and how far this corpus has taken it. Decoding is by far the
largest family and, with structural analysis and tracing, the oldest; discovery is two years old and
intervention three. Table~\ref{tab:famreq} shows two asymmetries. Intervention gives the claims of
greatest practical interest at a high cost and with the smallest controlled fraction, while the cheapest family addresses
