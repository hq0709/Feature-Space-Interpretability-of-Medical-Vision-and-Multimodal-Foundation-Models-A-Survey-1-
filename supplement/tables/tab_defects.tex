...
\caption{The defect screen: the five recorded result-invalidating defect kinds, and the \Fdefrec{} records that carry one. \Fflagrec{} of those are flagged. A record is flagged when its study carries one of these defects \emph{and} the defect touches that record's own claim (or its applicability is uncertain); a defect concerning another record of the same study leaves this one unmarked. The screen is the review's own rule, fixed before the findings were assembled; it is not a risk-of-bias instrument, and the article counts every record, flagged or not, and the profile tables give the corpus-breadth tier each finding would receive with the flagged records set aside.}
\label{tab:defects}
...
\midrule
Total & records with a recorded defect of these kinds & \Fdeftouch{} touched, \Fdefunc{} uncertain, \Fdefstay{} left in & \Fdefrec{} \\
\bottomrule
\end{tabular}
\end{table}
