...
design, which no reviewed medical study reports (\S\ref{sec:causal-claims}). The methods come from language
...
representations, but concept-directed effects are specific to the input and the model. A matched control applies to the eight records of F9a, those that target a specific direction, feature,
unit or token; four of them target a named concept, and three report a matched control. The other thirteen records harmonise, project, erase or edit without such a target, share no estimand
with each other, and are inventoried under F9b, whose fourteen studies are not pooled into a family
total. No study in this corpus intervenes on the connector, and
